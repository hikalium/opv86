var OpV86 = /** @class */ (function () {
    function OpV86() {
    }
    OpV86.prototype.isMatchedWithFilter = function (op, filter) {
        if (filter.length == 0)
            return true;
    ...
    OpV86.prototype.updateTable = function (data, filter) {
        var oplist = $('#oplist');
        oplist.empty();
        ...
        for (var _i = 0, _a = data.ops; _i < _a.length; _i++) {
            ...
            var opcodeBytes = op.opcode.replace(/REX\.W \+/g, 'REX.W')
                .replace(/REX \+/g, 'REX')
                .replace(/ \+ /g, '+')
                .replace(/\+ /g, '+')
                .replace(/ \+/g, '+')
...
$.getJSON("data/ops.json", function (data) {
    ...
    */
    document.getElementById('filter-value').addEventListener('keyup', function () {
        opv86.updateTable(data, document.getElementById('filter-value').value);
    });
